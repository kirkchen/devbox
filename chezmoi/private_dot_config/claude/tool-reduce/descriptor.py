...
預算很充裕。寫得清楚才有逐段還原，而逐段還原是整個效益成立的前提 —— 離線研究
量到的是「整份重讀」會讓淨效益從 +3.6% 掉到 −0.9%，也就是由正轉負。那兩個
絕對值出自另一份 chunker 的離線研究，跟上線後實測的 2.2% 不是同一組數字
（見 drop_policy.py 與 README）；這裡要的是那個符號會翻轉的結論，不是數值本身。
"""
import re
